...
do $v80_postflight$
declare v_name text; v_actual text; v_default text; v_role text; v_priv text; v_allowed boolean; v_count integer; v_constraint pg_constraint%rowtype;
begin
  ...
  if not found or v_constraint.contype IS DISTINCT FROM 'p' or v_constraint.conkey IS DISTINCT FROM array[(select attnum from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname='video_item_id')] or v_constraint.convalidated IS DISTINCT FROM true then raise exception using errcode='P0001',message='v80_schema_collision'; end if;

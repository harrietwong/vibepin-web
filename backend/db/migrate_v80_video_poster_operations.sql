...
do $v80_postflight$
declare v_name text; v_actual text; v_role text; v_priv text; v_allowed boolean; v_count integer; v_constraint pg_constraint%rowtype;
begin
  select * into v_constraint from pg_constraint where conrelid='public.video_poster_operations'::regclass and conname='video_poster_operations_pkey';
  if not found or v_constraint.contype<>'p' or v_constraint.conkey<>array[(select attnum from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname='video_item_id')] or not v_constraint.convalidated then raise exception using errcode='P0001',message='v80_schema_collision'; end if;
  select * into v_constraint from pg_constraint where conrelid='public.video_poster_operations'::regclass and conname='video_poster_operations_video_item_id_fkey';
  if not found or v_constraint.contype<>'f' or v_constraint.confrelid<>'public.video_upload_items'::regclass or v_constraint.conkey<>array[(select attnum from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname='video_item_id')] or v_constraint.confkey<>array[(select attnum from pg_attribute where attrelid='public.video_upload_items'::regclass and attname='id')] or v_constraint.confupdtype<>'a' or v_constraint.confdeltype<>'r' or v_constraint.confmatchtype<>'s' or v_constraint.condeferrable or v_constraint.condeferred or not v_constraint.convalidated then raise exception using errcode='P0001',message='v80_schema_collision'; end if;
  select * into v_constraint from pg_constraint where conrelid='public.video_poster_operations'::regclass and conname='video_poster_operations_bucket_id_object_path_key';
  if not found or v_constraint.contype<>'u' or v_constraint.conkey<>array[(select attnum from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname='bucket_id'),(select attnum from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname='object_path')] or not v_constraint.convalidated then raise exception using errcode='P0001',message='v80_schema_collision'; end if;
  ...
  if not found or v_actual <> $v80_state$CHECK (state = ANY (ARRAY['associated'::text, 'retained'::text]))$v80_state$ then raise exception using errcode='P0001',message='v80_schema_collision'; end if;
  ...
  if not found or v_actual <> $v80_path$CHECK (object_path ~ '^studio/uploads/[0-9A-Fa-f-]{8,64}/[A-Za-z0-9][A-Za-z0-9_.-]{0,200}\.(png|jpg|jpeg|webp|gif)$'::text)$v80_path$ then raise exception using errcode='P0001',message='v80_schema_collision'; end if;
  if exists(select 1 from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname in ('video_item_id','owner_user_id','bucket_id','object_path','state','created_at','updated_at') and not attnotnull)
    or (select pg_get_expr(adbin,adrelid) from pg_attrdef where adrelid='public.video_poster_operations'::regclass and adnum=(select attnum from pg_attribute where attrelid='public.video_poster_operations'::regclass and attname='state')) <> '''associated''::text' then raise exception using errcode='P0001',message='v80_schema_collision'; end if;

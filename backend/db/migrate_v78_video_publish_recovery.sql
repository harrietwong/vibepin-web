...
begin
  ...
  if exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname like '%\_v78' escape '\'
      and not exists (select 1 from unnest(array[
        to_regprocedure('public.publish_intent_confirm_prepare_v78(uuid,jsonb,text)'),
        to_regprocedure('public.publish_asset_claim_ready_v78(uuid,text,text,uuid)'),
        to_regprocedure('public.publish_asset_ready_sources_v78(uuid,text,text)'),
        to_regprocedure('public.publish_provider_attempt_settle_v78(uuid,uuid,uuid,text,integer,text,text,jsonb)')
      ]) expected(oid) where expected.oid=p.oid)) then
    raise exception using errcode='P0001',message='v78_definition_tamper';
  end if;
...
begin
  if p_source_identity_fingerprint !~ '^[0-9a-f]{64}$' then

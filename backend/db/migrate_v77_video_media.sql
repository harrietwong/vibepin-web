...
begin
  ...
  select * into v_batch from public.video_upload_batches
   where owner_user_id=p_owner_user_id and idempotency_key=btrim(p_idempotency_key) for update;
  if not found then
    insert into public.video_upload_batches(owner_user_id,idempotency_key,expires_at)
      values(p_owner_user_id,btrim(p_idempotency_key),p_expires_at) returning * into v_batch;
  end if;
...
begin
  ...
  if v_batch.expires_at<=now() then
    update public.video_upload_batches set status='expired',updated_at=now() where id=v_batch.id;
    raise exception using errcode='55000',message='video_upload_batch_expired';
  end if;
  ...
  if found then
    if v_item.idempotency_key is distinct from btrim(p_idempotency_key)
       ...
       or v_item.declared_byte_size is distinct from p_declared_byte_size then
...
begin
  if p_verified_content_type not in ('video/mp4','video/x-m4v','video/quicktime') then raise exception using errcode='22023',message='invalid_video_content_type'; end if;
  ...
  if v_item.status='finalized' then
    if v_item.verified_content_type is distinct from p_verified_content_type or v_item.verified_byte_size is distinct from p_verified_byte_size
       or v_item.verified_checksum_sha256 is distinct from nullif(btrim(coalesce(p_verified_checksum_sha256,'')),'') then
...
declare v_signature text; v_definition text; v_old text := '(''image/png'',''image/jpeg'',''image/webp'')'; v_new text := '(''image/png'',''image/jpeg'',''image/webp'',''video/mp4'',''video/x-m4v'',''video/quicktime'')';
begin
  foreach v_signature in array array[
    ...
    if v_definition is null or obj_description(to_regprocedure(v_signature),'pg_proc') !~ '^vibepin:v76:' then

...
| Stable entity, lifecycle and history | Canonical URL/hash identity, partial current uniqueness, database-owned lifecycle transitions and history-preserving retirement are enforced. Exact PGlite transactions prove retired/current coexistence and complete schema rollback. An isolated native PostgreSQL 17.11 replay passed the exact schema contract and exercised the partial unique index across two sessions with zero persisted rows, locks or sessions. A read-only Management API preflight also identified a healthy PostgreSQL 17.6 VibePin test project with zero v3.7 objects and every direct migration/canary prerequisite. | The test project lacks production's two legacy Product tables, so it can prove only Supabase multi-session/RLS/rollback behavior, not production legacy integrity. The canary has not yet executed; its exact migration/canary/rollback sequence requires external-write authority, and production v63 schema remains absent. | Implementation + native PostgreSQL PASS / Test platform READ-ONLY READY |
...
| Saved Products is a record, not a tracking trigger | Saved Products is an account relation with database-owned idempotent state/history. UI copy describes a shortlist. Tracking has no Saved dependency. The rollback-only native PostgreSQL replay used two fixed fake authenticated identities and anon, proving per-user reads and server-only writes with zero persisted rows. The selected isolated test project has 19 users, enough for the same two-user rollback probe, without exposing their identities. | The same path through Supabase Management API sessions remains pending explicit test-project write authority; production relation is absent until Stage 1. | Implementation + native PostgreSQL PASS / Test platform READ-ONLY READY |

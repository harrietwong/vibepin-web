...
const stateRoot = path.join(process.env.TEMP || here, "vibepin-claude-dispatcher-recursion-test");
...
try {
  ...
  const err = await request("tools/call", {
    name: "claude_dispatch",
    arguments: {
      role: "opus",
      mode: "read_only",
      task_name: "recursion-probe",
      task: "should be refused",
    },
  }).catch((e) => e);
  assert.match(err.message, /recursion guard/);
  console.log(
    "Recursion guard test passed: claude_dispatch refused under CODEX_CALLED_FROM_CLAUDE=1 (no paid call).",
  );
} finally {
  child.kill();
}

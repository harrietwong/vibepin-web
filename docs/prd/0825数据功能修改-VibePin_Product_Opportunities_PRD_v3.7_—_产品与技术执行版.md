...
candidate systemd units = NEEDS CURRENT VPS EVIDENCE; 2026-08-27T23:45:11Z 的 VPS `/tmp` 六 unit/四 wrapper/systemd 255 PASS 属于 6839e760 历史候选。9a22c16 的 systemd/wrapper bytes 与 1946a684 相同，但都未获得当前候选名义下的 VPS 回执，因此部署前必须重新暂存精确当前 6+4 文件、逐 SHA-256 回读并跑 systemd-analyze verify；旧回执不能改名复用。旧证据为 `backend/docs/product_opportunities_v37_platform_preflight_20260827T234511Z.json`

...
Vercel project binding = BLOCK; authenticated Vercel CLI read-back at 2026-08-27T17:36:28Z proves the current project uses Root Directory `.` and install command `npm install`, while the latest Ready production build used Next.js 16.2.6. This contradicts the reviewed Root Directory `web`, `npm ci`, Next.js 16.3.3 candidate boundary, and no immutable build exists for functional candidate 6839e760. A separately authorized platform correction and exact candidate build-log read-back are required before promotion
...
2026-08-27T17:36:28Z 使用已认证 Vercel CLI 58.4.0 完成只读回读，平台证据缺口已经关闭，但结果明确不合格：Root Directory=`.`、Install Command=`npm install`，最近一次 Ready 生产构建使用 Next.js 16.2.6 且早于当前候选。不得把“最近生产构建成功”冒充 6839e760 候选通过；在单独授权修改 Vercel 配置并得到候选不可变构建日志前，Web promotion 保持 BLOCK。证据：`backend/docs/product_opportunities_v37_vercel_platform_block_20260827T173628Z.json`。

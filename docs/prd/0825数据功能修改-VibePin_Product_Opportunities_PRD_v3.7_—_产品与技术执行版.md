...
real PostgreSQL concurrency/RLS canary = NATIVE PG17 PASS / NEEDS SUPABASE TEST-PROJECT EVIDENCE; a299a17 provides a default-zero-network, rollback-only tool bound to exact project/manifest/migration bytes. An isolated PostgreSQL 17.11 instance bound to 127.0.0.1 executed two-session active-identity uniqueness, authenticated/anon Saved Products isolation, server-only writes and the exact P0001 rollback sentinel; before/after Product/Saved counts were zero, advisory locks/sessions were zero, schema rollback left zero v63 objects and the harness removed its fixtures. Separately labelled operator lifecycle evidence records the portable runtime archive, fixed replay-database drop, server stop and final pg_isready no-response result; those lifecycle fields are not emitted by the replay harness. A production SELECT-only probe confirmed PostgreSQL 17.6, matching the replay major, and separately proved the Management API message-only wrapper with embedded SQLSTATE. Evidence: `backend/docs/product_opportunities_v37_local_postgres_replay_20260828T084744Z.json`, `backend/docs/product_opportunities_v37_production_postgres_version_probe_20260828T083053Z.json`, and `backend/docs/product_opportunities_v37_management_api_error_shape_probe_20260828T073558Z.json`. The isolated Supabase test-project run remains mandatory to prove the Management API multi-session path; no production concurrency/RLS mutation occurred

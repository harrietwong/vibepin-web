...
```text
release topology = PASS for the local Product-only candidate; functional commit a299a17dae428dae69d55f4262f5301804fb35e7 descends from production remote b22930ebe73847cf35bc44be789414902ae6b599 and remains bounded by the exact 81-artifact Product manifest; the superseded 99efabc whole-tree candidate must not be deployed because it also carries unrelated Usage/Metering production files
backend = full suite PASS; 1011 passed, 2 live-credential tests skipped from the clean Product-only committed state
...
release manifest contract = PASS; the current a299a17 manifest/automation contract passed 30/30
shell wrappers = PASS locally; ShellCheck 0.11.0 passed the four exact LF Git blobs inherited unchanged by a299a17 for cloud_lib, Product Supply, Product Opportunity Admission and Product Tracking, excluding only the intentional dynamic-source SC1091 diagnostic; focused Admission/canary checks passed 134/134. The Admission wrapper changed after the earlier VPS `/tmp` preflight, so exact current-candidate staging/hash/systemd-analyze verification remains a mandatory pre-install host gate

candidate systemd units = NEEDS CURRENT VPS EVIDENCE; 2026-08-27T23:45:11Z 的 VPS `/tmp` 六 unit/四 wrapper/systemd 255 PASS 属于 6839e760 历史候选。a299a17 的 systemd/wrapper bytes 与 1946a684 相同，但都未获得当前候选名义下的 VPS 回执，因此部署前必须重新暂存精确当前 6+4 文件、逐 SHA-256 回读并跑 systemd-analyze verify；旧回执不能改名复用。旧证据为 `backend/docs/product_opportunities_v37_platform_preflight_20260827T234511Z.json`
local candidate systemd boundary = PASS for current wrapper lint only; a299a17 继承的四个精确 LF Git blob 已通过 ShellCheck 0.11.0，聚焦 Admission/canary 134/134；历史 6839e760 的六 unit/四 wrapper 本地证据保留于 `backend/docs/product_opportunities_v37_local_systemd_gate_20260827T174000Z.json`，不得替代当前 VPS systemd-analyze
...
real PostgreSQL concurrency/RLS canary = NATIVE PG17 PASS / SUPABASE TEST PROJECT READ-ONLY READY / EXECUTION NEEDS AUTHORITY; a299a17 provides a default-zero-network, rollback-only tool bound to exact project/manifest/migration bytes. An isolated PostgreSQL 17.11 instance bound to 127.0.0.1 executed two-session active-identity uniqueness, authenticated/anon Saved Products isolation, server-only writes and the exact P0001 rollback sentinel; before/after Product/Saved counts were zero, advisory locks/sessions were zero, schema rollback left zero v63 objects and the harness removed its fixtures. Separately labelled operator lifecycle evidence records the portable runtime archive, fixed replay-database drop, server stop and final pg_isready no-response result; those lifecycle fields are not emitted by the replay harness. A production SELECT-only probe confirmed PostgreSQL 17.6, matching the replay major, and separately proved the Management API message-only wrapper with embedded SQLSTATE. A Management API metadata/catalog-only preflight found one healthy non-production project whose undisclosed name identifies it as VibePin test, PostgreSQL 17.6, 19 test users and zero v63 objects; it has `auth.users`, `auth.uid()`, anon/authenticated roles and pgcrypto required by the migration/canary. It does not have the production legacy `pin_products` or `pin_save_snapshots` relations, so it is valid only for the Supabase multi-session/RLS/rollback platform proof and must not be used to claim production legacy-integrity verification. No identities or business rows were read and no database mutation occurred. Evidence: `backend/docs/product_opportunities_v37_local_postgres_replay_20260828T084744Z.json`, `backend/docs/product_opportunities_v37_production_postgres_version_probe_20260828T083053Z.json`, `backend/docs/product_opportunities_v37_management_api_error_shape_probe_20260828T073558Z.json`, and `backend/docs/product_opportunities_v37_test_project_readiness_20260828T094518Z.json`. Applying the exact migration, executing the rollback-only Management API canary and rolling the test schema back still require explicit external-write authority; no production concurrency/RLS mutation occurred

...
```text
release topology = PASS for the local Product-only candidate; functional commit 1946a68483f7ca225438d7a98c6f897ee7f088c5 descends from production remote b22930ebe73847cf35bc44be789414902ae6b599 and remains bounded by the exact 80-artifact Product manifest; the superseded 99efabc whole-tree candidate must not be deployed because it also carries unrelated Usage/Metering production files
backend = full suite PASS; 966 passed, 2 live-credential tests skipped from the clean Product-only committed state
...
release manifest contract = PASS; the current 1946a684 manifest/automation contract passed 30/30
shell wrappers = PASS locally; ShellCheck 0.11.0 passed the exact 1946a684 LF Git blobs for cloud_lib, Product Supply, Product Opportunity Admission and Product Tracking, excluding only the intentional dynamic-source SC1091 diagnostic; focused Admission/migration checks passed 132/132. The Admission wrapper changed after the earlier VPS `/tmp` preflight, so exact 1946a684 staging/hash/systemd-analyze verification remains a mandatory pre-install host gate

candidate systemd units = NEEDS CURRENT VPS EVIDENCE; 2026-08-27T23:45:11Z 的 VPS `/tmp` 六 unit/四 wrapper/systemd 255 PASS 属于 6839e760 历史候选。1946a684 的 unit 未变但 Admission wrapper 已变，因此部署前必须重新暂存精确当前 6+4 文件、逐 SHA-256 回读并跑 systemd-analyze verify；旧回执不能改名复用。旧证据为 `backend/docs/product_opportunities_v37_platform_preflight_20260827T234511Z.json`
local candidate systemd boundary = PASS for current wrapper lint only; 1946a684 四个精确 LF Git blob 已通过 ShellCheck 0.11.0，聚焦 Admission/migration 132/132；历史 6839e760 的六 unit/四 wrapper 本地证据保留于 `backend/docs/product_opportunities_v37_local_systemd_gate_20260827T174000Z.json`，不得替代当前 VPS systemd-analyze

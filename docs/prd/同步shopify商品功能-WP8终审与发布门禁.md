...
| 4 | 注册 3 个 GDPR compliance webhooks（CLI：config link → deploy，版本 `vibepin-5`，App URL 同时修正回 /launch） | 我 | ✅ 2026-07-12 完成 |
| 5 | Vercel Production env 核对：`SHOPIFY_REDIRECT_URI=https://vibepin.co/api/integrations/shopify/callback`、`SHOPIFY_APP_URL=https://vibepin.co`（此前曾误填一次性部署域名） | 用户 | ✅ 已改（2026-07-12 用户确认） |
| 6 | 部署代码到生产（从干净 git HEAD worktree 部署，不含其他会话未提交工作；ASCII 路径 + hostname preload 配方） | 我 | ✅ 2026-07-12 已上线 vibepin.co；4 个安全探针（webhook 坏签名/launch 无 HMAC/status/sync 无鉴权）全部 401 |

...
import {
  ...
  Bookmark, ImageIcon, Search, Lightbulb, Wand2, CalendarDays, Star,
} from "lucide-react";
...
type Status      = "Ready" | "Rising" | "Planned";
...
interface HeroDay { day: string; date: number; title: string; status: Status; img: string; }
...
const STATS = [
  { value: "Daily",  label: "opportunity signals" },
  { value: "18+",    label: "categories tracked" },
  { value: "100+",   label: "Pin formats & angles" },
  { value: "7-day",  label: "visual content planner" },
];
...
const STATUS_STYLE: Record<Status, { bg: string; color: string }> = {
  ...
  "Rising":  { bg: "rgba(217,70,239,0.18)", color: "#E879F9" },
...
const PRICING = [
  { plan: "Free",    monthly: "$0",  yearly: "$0",  period: "",    planKey: "free",
    ...
    features: ["Top opportunities (limited)", "Pin Ideas (limited)", "Product Opportunities (limited)", "Manual export"], highlighted: false, cta: "Get started" },
  { plan: "Creator", monthly: "$19", yearly: "$15", period: "/mo", planKey: "creator",
    desc: "Find, create, and plan as a solo creator.",
    features: ["Full opportunity feed", "Pin Ideas + references", "Create Pins (150/mo)", "Weekly Plan board"], highlighted: false, cta: "Start free trial" },
  { plan: "Growth",  monthly: "$49", yearly: "$39", period: "/mo", planKey: "growth",
    desc: "Scale product-aware content for your store.",
    features: ["Everything in Creator", "Create Pins (500/mo)", "Linked-product Pins + URLs", "Priority support"], highlighted: true, cta: "Start free trial" },
  { plan: "Agency",  monthly: "$99", yearly: "$79", period: "/mo", planKey: "pro",
    desc: "Plan Pinterest content for multiple brands.",
    features: ["Everything in Growth", "Team workspace (3 seats)", "Unlimited Weekly Plans", "API access (coming soon)"], highlighted: false, cta: "Start free trial" },
];
...
function Pill({ label, tone }: { label: string; tone: "green" | "cyan" | "amber" | "magenta" }) {
  const map = {
    ...
    magenta: { bg: "rgba(217,70,239,0.15)", color: "#E879F9" },
  }[tone];
  ...
  return (
    <div className="rounded-xl border overflow-hidden transition-colors" style={{ borderColor: open ? "rgba(217,70,239,0.30)" : "rgba(255,255,255,0.08)", background: open ? "rgba(217,70,239,0.04)" : "transparent" }}>
      <button type="button" onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-5 py-4 text-left text-[14px] font-semibold transition-colors hover:text-white" style={{ color: "#D1D5DB" }}>
        {q}
        <span className={`ml-4 text-xl leading-none transition-transform duration-200 shrink-0 ${open ? "rotate-45" : ""}`} style={{ color: open ? "#E879F9" : "#4B5563" }}>+</span>
      </button>
...
// Horizontal rail with manual arrows (scroll-snap)
function Rail({ children, accent }: { children: React.ReactNode; accent: string }) {
  ...
  return (
    <div className="relative">
      <div ref={ref} className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory no-scrollbar" style={{ scrollbarWidth: "none" }}>
        {children}
      </div>
      <button type="button" onClick={() => scroll(-1)} aria-label="Previous"
        className="hidden sm:flex absolute -left-4 top-1/2 -translate-y-1/2 h-9 w-9 items-center justify-center rounded-full border backdrop-blur transition-transform hover:scale-110"
        style={{ background: "rgba(8,14,11,0.85)", borderColor: `${accent}55`, color: accent }}>
        <ArrowLeft className="w-4 h-4" />
      </button>
      <button type="button" onClick={() => scroll(1)} aria-label="Next"
        className="hidden sm:flex absolute -right-4 top-1/2 -translate-y-1/2 h-9 w-9 items-center justify-center rounded-full border backdrop-blur transition-transform hover:scale-110"
        style={{ background: "rgba(8,14,11,0.85)", borderColor: `${accent}55`, color: accent }}>
...
export default function HomePage() {
  ...
  useEffect(() => {
    const h = () => setShowSticky(window.scrollY > 640);
  ...
  return (
    <div className="lp min-h-screen antialiased" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      <nav className="sticky top-0 z-50 border-b backdrop-blur-md" style={{ background: "rgba(8,14,11,0.9)", borderColor: "rgba(255,255,255,0.07)" }}>
        <div className="max-w-[1240px] mx-auto px-5 h-[60px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <Link href="/app/discover?demo=true" className={`${VibeBtn} px-4 py-2 text-[13px] flex items-center gap-1.5`}>Build my 7 Pins <ArrowRight className="w-3.5 h-3.5" /></Link>
          </div>
        </div>
      </nav>

      {/* ══ HERO ═══════════════════════════════════════════════════════════════ */}
      <section className="relative pt-16 pb-14 lg:pt-24 lg:pb-20 overflow-hidden">
        <div className="pointer-events-none absolute -top-32 right-[-10%] h-[480px] w-[480px] rounded-full blur-3xl" style={{ background: "radial-gradient(circle, rgba(217,70,239,0.16), transparent 70%)" }} />
        <div className="max-w-[1240px] mx-auto px-5 grid grid-cols-1 lg:grid-cols-[0.95fr_1.1fr] gap-12 items-center relative">
          {/* Left */}
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 mb-6" style={{ background: "rgba(217,70,239,0.08)", borderColor: "rgba(217,70,239,0.25)" }}>
              ...
              <span className="text-[11px] font-semibold tracking-wide" style={{ color: "#E879F9" }}>AI-powered Pinterest workflow</span>
            </div>
            <h1 className="text-[2.6rem] sm:text-[3.4rem] lg:text-[3.7rem] font-black text-white leading-[1.02] tracking-[-0.045em] mb-5">
              Discover. Create.<br />Plan. Grow on<br />
              <span style={{ background: "linear-gradient(100deg,#FF4D8D,#D946EF 60%,#A855F7)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>Pinterest.</span>
            </h1>
            <p className="text-[15px] sm:text-[16px] leading-relaxed mb-8 max-w-[440px]" style={{ color: "#8B93A1" }}>
              VibePin helps creators and businesses find opportunities, generate high-performing Pins, and plan content that drives results.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 mb-7">
              ...
              <a href="#create" className="flex items-center justify-center gap-2 rounded-full px-7 py-3.5 text-[14px] font-semibold border transition-colors hover:text-white hover:border-white/30" style={{ color: "#9097A0", borderColor: "rgba(255,255,255,0.14)" }}>See how it works</a>
            </div>
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
              {["Opportunity intelligence", "Product-aware AI", "One-click planning"].map(t => (
          ...
          {/* Right — Weekly Plan dashboard preview */}
          <div className="rounded-2xl border overflow-hidden shadow-2xl" style={{ background: "#0C1018", borderColor: "rgba(255,255,255,0.10)" }}>
            <div className="flex items-center justify-between px-4 py-3 border-b" style={{ background: "#0A0E16", borderColor: "rgba(255,255,255,0.06)" }}>
              <div className="flex items-center gap-2">
                <BrandLogo size={20} />
                <div>
                  <p className="text-[12px] font-bold text-white leading-none">Weekly Plan</p>
                  <p className="text-[9px] mt-0.5" style={{ color: "#4B5563", ...MONO }}>May 6 – May 12, 2026</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold border" style={{ color: "#9097A0", borderColor: "rgba(255,255,255,0.12)" }}>Add Ideas</span>
                <span className="rounded-full px-2.5 py-1 text-[10px] font-bold text-white" style={{ background: "linear-gradient(135deg,#D946EF,#7C3AED)" }}>Generate Pins</span>
              </div>
            </div>
            {/* Stat chips */}
            <div className="grid grid-cols-4 gap-px px-px" style={{ background: "rgba(255,255,255,0.05)" }}>
              {[{ v: "18", l: "Pins this week", c: "#fff" }, { v: "+132%", l: "vs last week", c: "#10B981" }, { v: "Daily", l: "Fresh signals", c: "#38BDF8" }, { v: "94", l: "Top opportunity", c: "#E879F9" }].map(s => (
                <div key={s.l} className="px-3 py-3 text-center" style={{ background: "#0C1018" }}>
                  <p className="text-[16px] font-black tabular-nums" style={{ color: s.c, ...MONO }}>{s.v}</p>
                  <p className="text-[8px] mt-0.5" style={{ color: "#4B5563" }}>{s.l}</p>
                </div>
              ))}
            </div>
            {/* 4-day grid */}
            <div className="grid grid-cols-4 gap-px px-px" style={{ background: "rgba(255,255,255,0.05)" }}>
              {HERO_DAYS.map(d => (
                <div key={d.day} className="p-2" style={{ background: "#0C1018" }}>
                  <p className="text-[9px] font-bold mb-1.5" style={{ color: "#4B5563" }}>{d.day} {d.date}</p>
                  <div className="relative rounded-lg overflow-hidden mb-1.5" style={{ aspectRatio: "3/4" }}>
                    <Image src={u(d.img, 160, 210)} alt={d.title} fill className="object-cover" sizes="120px" unoptimized />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                    <p className="absolute bottom-1 left-1.5 right-1.5 text-[8px] font-bold text-white leading-tight">{d.title}</p>
                  </div>
                  <span className="inline-block rounded-full px-1.5 py-0.5 text-[7px] font-bold" style={{ background: STATUS_STYLE[d.status].bg, color: STATUS_STYLE[d.status].color }}>{d.status}</span>
                </div>
              ))}
            </div>
            {/* Generated pin row */}
            <div className="flex items-center gap-2.5 px-4 py-3 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", background: "#0A0E16" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[9px]" style={{ color: "#4B5563" }}>Room Color Palette Ideas</p>
              </div>
              <svg viewBox="0 0 80 24" className="h-6 w-20" fill="none"><polyline points="0,20 14,16 28,18 42,9 56,12 70,4 80,6" stroke="#10B981" strokeWidth="2" /></svg>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ══ BRAND ROW ══════════════════════════════════════════════════════════ */}
      <section className="border-y py-6" style={{ borderColor: "rgba(255,255,255,0.07)", background: "var(--surface)" }}>
        <p className="text-center text-[11px] font-semibold uppercase tracking-[0.18em] mb-5" style={{ color: "#4B5563" }}>Works with the tools you already sell on</p>
        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-4 px-5">
          {BRAND_ROW.map(b => (
            <span key={b} className="text-[18px] font-black tracking-tight transition-opacity hover:opacity-100" style={{ color: "#5B6472", opacity: 0.7 }}>{b}</span>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="max-w-[900px] mx-auto px-5 relative">
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl sm:text-[2.4rem] font-black text-white tracking-tight leading-[1.12]">
              Most tools help you make more.<br />VibePin helps you decide what&apos;s worth making.
            </h2>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-2xl border p-6" style={{ background: "rgba(220,38,38,0.04)", borderColor: "rgba(220,38,38,0.18)" }}>
              <div className="flex items-center gap-2.5 mb-4">
                <span className="h-7 w-7 rounded-full flex items-center justify-center" style={{ background: "rgba(220,38,38,0.15)" }}><X className="w-4 h-4" style={{ color: "#F87171" }} /></span>
                <div><p className="text-[14px] font-bold text-white">Generate more content</p><p className="text-[11px]" style={{ color: "#6B7280" }}>More volume, more guessing.</p></div>
              </div>
              <ul className="space-y-2">{["No demand signals", "Hard to know what people want", "More content, less results"].map(t => (
                <li key={t} className="flex items-center gap-2 text-[12px]" style={{ color: "#6B7280" }}><span className="h-1 w-1 rounded-full" style={{ background: "#6B7280" }} />{t}</li>
              ))}</ul>
            </div>
            <div className="rounded-2xl border p-6" style={{ background: "rgba(124,58,237,0.07)", borderColor: "rgba(168,85,247,0.30)" }}>
              <div className="flex items-center gap-2.5 mb-4">
                <span className="h-7 w-7 rounded-full flex items-center justify-center" style={{ background: "rgba(168,85,247,0.20)" }}><Check className="w-4 h-4" style={{ color: "#C4B5FD" }} /></span>
                <div><p className="text-[14px] font-bold text-white">Find it, create it, plan it</p><p className="text-[11px]" style={{ color: "#9097A0" }}>One workflow from signal to scheduled Pin.</p></div>
              </div>
              <ul className="space-y-2">{["Real Pinterest demand signals", "Product-aware Pin creation", "Weekly content planning"].map(t => (
                <li key={t} className="flex items-center gap-2 text-[12px]" style={{ color: "#C8CDD6" }}><Check className="w-3 h-3 shrink-0" style={{ color: "#A855F7" }} />{t}</li>
              ))}</ul>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-[1060px] mx-auto px-5 grid lg:grid-cols-[0.8fr_1.2fr] gap-10 items-center">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="h-6 w-6 rounded-full flex items-center justify-center text-[11px] font-black" style={{ background: "rgba(8,145,178,0.15)", color: "#38BDF8" }}>1</span>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: "#38BDF8" }}>Intelligence</p>
            </div>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ background: "#0C1018", borderColor: "rgba(255,255,255,0.10)" }}>
            ...
            {INTEL_ROWS.map(r => (
              <div key={r.rank} className="grid items-center px-5 py-3 border-b last:border-0 transition-colors hover:bg-white/[0.02]"
                style={{ gridTemplateColumns: "1fr 4.5rem 5rem 4rem 4rem", gap: "0.75rem", borderColor: "rgba(255,255,255,0.04)" }}>
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-[11px] tabular-nums" style={{ color: "#374151", ...MONO }}>{r.rank}</span>
      ...
      {/* ══ PIN IDEAS ══════════════════════════════════════════════════════════ */}
      <section id="pin-ideas" className="py-16 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="max-w-[1100px] mx-auto px-5">
          <div className="flex items-end justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="h-6 w-6 rounded-full flex items-center justify-center text-[11px] font-black" style={{ background: "rgba(217,70,239,0.15)", color: "#E879F9" }}>2</span>
                <p className="text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: "#E879F9" }}>Pin Ideas</p>
              </div>
              ...
              <div key={i} className="snap-start shrink-0 rounded-xl overflow-hidden group/pin transition-transform hover:-translate-y-1" style={{ width: 188, background: "#0C1018", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div className="p-2.5">
                  ...
                  <div className="flex flex-wrap gap-1">
                    <Pill label={p.demand === "High Demand" ? "High signal" : "Rising"} tone="green" />
                    <Pill label={p.competition === "Low Competition" ? "Low comp" : "Moderate"} tone={p.competition === "Low Competition" ? "cyan" : "amber"} />
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="products" className="py-16 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", background: "var(--surface)" }}>
        <div className="max-w-[1100px] mx-auto px-5">
          <div className="flex items-end justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="h-6 w-6 rounded-full flex items-center justify-center text-[11px] font-black" style={{ background: "rgba(16,185,129,0.15)", color: "#10B981" }}>3</span>
                <p className="text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: "#10B981" }}>Product Opportunities</p>
              </div>
                ...
                <div key={i} className="snap-start shrink-0 rounded-xl overflow-hidden transition-transform hover:-translate-y-1" style={{ width: 188, background: "#0C1018", border: "1px solid rgba(255,255,255,0.08)" }}>
                  ...
                  <div className="p-2.5">
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      <Pill label={p.demand === "High Demand" ? "High Demand" : "Rising"} tone="green" />
                      <Pill label={p.competition === "Low Competition" ? "Low Comp" : "Moderate"} tone={p.competition === "Low Competition" ? "cyan" : "amber"} />
                    </div>
                    <div className="flex items-center gap-3 text-[9px] font-semibold pt-2 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", color: "#6B7280" }}>
                      <span className="flex items-center gap-1 hover:text-white transition-colors"><Bookmark className="w-2.5 h-2.5" /> Save</span>
                      <span className="flex items-center gap-1 hover:text-white transition-colors"><ImageIcon className="w-2.5 h-2.5" /> Use in Pins</span>
                    </div>
                  </div>
                </div>
              );
      ...
      {/* ══ CREATE + PLAN ══════════════════════════════════════════════════════ */}
      <section id="create" className="py-16 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="max-w-[1100px] mx-auto px-5">
          <div className="text-center mb-9">
            <div className="inline-flex items-center gap-2 mb-3">
              <span className="h-6 w-6 rounded-full flex items-center justify-center text-[11px] font-black" style={{ background: "rgba(251,146,60,0.15)", color: "#FB923C" }}>4</span>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: "#FB923C" }}>Create + Plan</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StepCard icon={<Plus className="w-4 h-4" />} accent="#FB923C" label="Add products">
              <div className="grid grid-cols-2 gap-1.5">
                {COMPOSER.products.concat("add").slice(0, 4).map((id, i) => id === "add"
                  ? <div key={i} className="rounded-lg flex items-center justify-center" style={{ aspectRatio: "1/1", border: "1px dashed rgba(255,255,255,0.14)", color: "#4B5563" }}><Plus className="w-4 h-4" /></div>
                  : <div key={i} className="relative rounded-lg overflow-hidden" style={{ aspectRatio: "1/1" }}><Image src={u(id, 90, 90)} alt="" fill className="object-cover" sizes="90px" unoptimized /></div>
                )}
              </div>
            </StepCard>
            {/* Add references */}
            <StepCard icon={<ImageIcon className="w-4 h-4" />} accent="#E879F9" label="Add references">
              <div className="grid grid-cols-2 gap-1.5">
                {COMPOSER.refs.concat("add").slice(0, 4).map((id, i) => id === "add"
                  ? <div key={i} className="rounded-lg flex items-center justify-center" style={{ aspectRatio: "1/1", border: "1px dashed rgba(255,255,255,0.14)", color: "#4B5563" }}><Plus className="w-4 h-4" /></div>
                  : <div key={i} className="relative rounded-lg overflow-hidden" style={{ aspectRatio: "1/1" }}><Image src={u(id, 90, 90)} alt="" fill className="object-cover" sizes="90px" unoptimized /></div>
                )}
              </div>
            ...
            <StepCard icon={<Wand2 className="w-4 h-4" />} accent="#A855F7" label="Generate Pins">
              <div className="grid grid-cols-2 gap-1.5">
                {COMPOSER.gen.map((g, i) => (
                  <div key={i} className="relative rounded-lg overflow-hidden" style={{ aspectRatio: "3/4" }}>
                    <Image src={u(g.img, 90, 120)} alt={g.title} fill className="object-cover" sizes="90px" unoptimized />
            ...
            <StepCard icon={<CalendarDays className="w-4 h-4" />} accent="#38BDF8" label="Plan your week">
              <div className="space-y-1.5">
                {COMPOSER.plan.map((d, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-lg p-1.5" style={{ background: "#080C12", border: "1px solid rgba(255,255,255,0.06)" }}>
                    <div className="relative rounded overflow-hidden shrink-0" style={{ width: 26, height: 26 }}><Image src={u(d.img, 52, 52)} alt="" fill className="object-cover" sizes="26px" unoptimized /></div>
                    <div className="min-w-0"><p className="text-[9px] font-bold text-white leading-none">{d.day}</p><p className="text-[8px] mt-0.5" style={{ color: "#4B5563" }}>10:00 AM</p></div>
                  </div>
                ))}
              </div>
            </StepCard>
          ...
          <div className="mt-6 text-center">
            <Link href="/app/discover?demo=true" className={`${VibeBtn} inline-flex items-center gap-2 px-7 py-3.5 text-[14px]`}>See it in action <ArrowRight className="w-4 h-4" /></Link>
          </div>
        </div>
      </section>

      {/* ══ STATS ══════════════════════════════════════════════════════════════ */}
      <section className="py-12 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", background: "var(--surface)" }}>
        <div className="max-w-[1100px] mx-auto px-5 grid grid-cols-2 lg:grid-cols-4 gap-px rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
          {STATS.map(s => (
            <div key={s.label} className="px-6 py-7 text-center" style={{ background: "var(--bg)" }}>
              <p className="text-4xl font-black tracking-tight mb-1" style={{ background: "linear-gradient(120deg,#FF4D8D,#A855F7)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent", ...MONO }}>{s.value}</p>
              <p className="text-[12px]" style={{ color: "#8B93A1" }}>{s.label}</p>
            </div>
          ))}
        </div>
        <div className="overflow-hidden mt-6 py-2.5 border-y" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
      ...
      <section className="py-20 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }} onMouseEnter={() => setUseCaseHover(true)} onMouseLeave={() => setUseCaseHover(false)}>
        <div className="max-w-[1100px] mx-auto px-5">
          <div className="text-center mb-8">
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] mb-3" style={{ color: "#A855F7" }}>Use VibePin for</p>
            <h2 className="text-3xl font-black text-white tracking-tight">Built for your kind of Pinterest.</h2>
          </div>
          <div className="flex items-center justify-center gap-2 mb-7 flex-wrap">
            {USE_CASES.map((c, i) => (
              <button key={c.cat} type="button" onClick={() => setUcIndex(i)}
                className="rounded-full px-4 py-2 text-[12px] font-semibold transition-all"
                style={i === ucIndex ? { background: "linear-gradient(135deg,#D946EF,#7C3AED)", color: "#fff" } : { background: "rgba(255,255,255,0.04)", color: "#9097A0", border: "1px solid rgba(255,255,255,0.08)" }}>
                {c.cat}
              </button>
            ))}
          </div>
          <div key={uc.cat} className="rounded-2xl border overflow-hidden grid lg:grid-cols-[0.85fr_1.15fr]" style={{ background: "#0C1018", borderColor: "rgba(255,255,255,0.10)" }}>
            <div className="p-7 flex flex-col justify-center">
              ...
              <div className="rounded-xl p-3.5 mb-5" style={{ background: "rgba(168,85,247,0.08)", border: "1px solid rgba(168,85,247,0.20)" }}>
                <p className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: "#C4B5FD" }}>The workflow</p>
                <p className="text-[12px] leading-relaxed" style={{ color: "#C8CDD6" }}>{uc.workflow}</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-1.5 p-3" style={{ background: "#0A0E16" }}>
              {uc.imgs.map((id, i) => (
                <div key={i} className="relative rounded-xl overflow-hidden" style={{ aspectRatio: "3/4" }}>
                  <Image src={u(id, 220, 293)} alt={uc.cat} fill className="object-cover" sizes="220px" unoptimized />
      ...
      <section className="py-20 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", background: "var(--surface)" }} onMouseEnter={() => setTestHover(true)} onMouseLeave={() => setTestHover(false)}>
        <div className="max-w-[1000px] mx-auto px-5">
          <div className="text-center mb-8">
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] mb-3" style={{ color: "#E879F9" }}>What creators say</p>
            <h2 className="text-3xl font-black text-white tracking-tight">Loved by creators, sellers, and agencies.</h2>
          </div>
          <div className="flex items-center justify-center gap-2 mb-8">
            {TEST_GROUPS.map(g => (
              <button key={g} type="button" onClick={() => setTestGroup(g)}
                className="rounded-full px-4 py-1.5 text-[12px] font-semibold transition-all"
                style={g === testGroup ? { background: "rgba(217,70,239,0.18)", color: "#E879F9", border: "1px solid rgba(217,70,239,0.35)" } : { background: "rgba(255,255,255,0.04)", color: "#9097A0", border: "1px solid rgba(255,255,255,0.08)" }}>
                {g}
              </button>
            ))}
          </div>
          {filteredTests.length > 0 && (
            <div key={testGroup + tIndex} className="max-w-2xl mx-auto rounded-2xl border p-8 text-center transition-opacity" style={{ background: "#0C1018", borderColor: "rgba(255,255,255,0.10)" }}>
              ...
              <p className="text-[18px] font-semibold text-white leading-relaxed mb-6" style={SERIF}>&ldquo;{filteredTests[tIndex % filteredTests.length].quote}&rdquo;</p>
              <div className="flex items-center justify-center gap-3">
                <span className="h-10 w-10 rounded-full flex items-center justify-center text-[14px] font-black text-white" style={{ background: "linear-gradient(135deg,#D946EF,#7C3AED)" }}>{filteredTests[tIndex % filteredTests.length].name.charAt(0)}</span>
                <div className="text-left">
                  <p className="text-[13px] font-bold text-white">{filteredTests[tIndex % filteredTests.length].name}</p>
                  <p className="text-[11px]" style={{ color: "#6B7280" }}>{filteredTests[tIndex % filteredTests.length].role}</p>
                </div>
              </div>
              <div className="flex justify-center gap-1.5 mt-6">
                {filteredTests.map((_, i) => (
                  <button key={i} type="button" onClick={() => setTIndex(i)} aria-label={`Testimonial ${i + 1}`}
                    className="h-1.5 rounded-full transition-all" style={{ width: i === tIndex % filteredTests.length ? 20 : 6, background: i === tIndex % filteredTests.length ? "#E879F9" : "rgba(255,255,255,0.15)" }} />
                ))}
              </div>
            </div>
          )}
        ...
        <div className="max-w-[1100px] mx-auto px-5">
          <div className="text-center mb-9">
            ...
            <div className="inline-flex items-center gap-1 rounded-full border p-1.5" style={{ background: "#080C12", borderColor: "rgba(255,255,255,0.08)" }}>
              {[{ label: "Monthly", val: false }, { label: "Yearly", val: true }].map(o => (
                <button key={o.label} type="button" onClick={() => setYearly(o.val)} className="rounded-full px-5 py-2 text-[13px] font-bold transition-all flex items-center gap-2"
                  style={yearly === o.val ? { background: "var(--surface-2)", color: "#E5E7EB" } : { color: "#4B5563" }}>
                  {o.label}{o.val && <span className="rounded-full px-2 py-0.5 text-[10px] font-bold text-white" style={{ background: "linear-gradient(135deg,#D946EF,#7C3AED)" }}>Save 20%</span>}
                </button>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 items-stretch">
            {PRICING.map(c => (
              <div key={c.plan} className="relative flex flex-col rounded-2xl p-6 transition-transform hover:-translate-y-1"
                style={c.highlighted ? { background: "linear-gradient(180deg,rgba(124,58,237,0.14),rgba(217,70,239,0.05))", border: "1px solid rgba(168,85,247,0.40)" } : { background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div className="flex items-end gap-1 mb-1">
                  <span className="text-4xl font-black text-white" style={MONO}>{yearly ? c.yearly : c.monthly}</span>
                  <span className="pb-1.5 text-sm" style={{ color: "#4B5563" }}>{c.period}</span>
                </div>
                <p className="text-[12px] mb-5 leading-relaxed" style={{ color: "#6B7280" }}>{c.desc}</p>
                <ul className="flex-1 space-y-2.5 mb-6">
                  {c.features.map(f => <li key={f} className="flex items-start gap-2.5 text-[12px]"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0" style={{ color: c.highlighted ? "#A855F7" : "#10B981" }} /><span style={{ color: "#C8CDD6" }}>{f}</span></li>)}
                </ul>
                <button type="button" onClick={() => router.push(c.planKey === "free" ? "/app/discover?demo=true" : `/signup?plan=${c.planKey}`)}
                  className={`w-full rounded-full py-3 text-[13px] font-bold transition-all ${c.highlighted ? VibeBtn : "border hover:text-white hover:border-white/30"}`}
                  style={c.highlighted ? {} : { borderColor: "rgba(255,255,255,0.14)", color: "#C8CDD6" }}>{c.cta}</button>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 relative overflow-hidden border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        <div className="max-w-2xl mx-auto px-5 text-center relative">
          <h2 className="text-4xl sm:text-5xl font-black text-white tracking-tight mb-4 leading-[1.05]">Your next best Pin is<br />one idea away.</h2>
          <p className="text-[15px] mb-9" style={{ color: "#8B93A1" }}>Find it, create it, and plan your week — in one workflow.</p>
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 border-t backdrop-blur-md px-5 py-3 flex items-center justify-between gap-4 transition-all duration-300 ${showSticky ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`} style={{ background: "rgba(8,12,18,0.95)", borderColor: "rgba(255,255,255,0.09)" }}>
        ...
        <Link href="/app/discover?demo=true" className={`${VibeBtn} shrink-0 px-5 py-2.5 text-[13px]`}>Build my 7 Pins</Link>
      </div>
    </div>
  );
}
...
function StepCard({ icon, accent, label, children }: { icon: React.ReactNode; accent: string; label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border p-4 transition-transform hover:-translate-y-1" style={{ background: "#0C1018", borderColor: "rgba(255,255,255,0.08)" }}>
      <div className="flex items-center gap-2 mb-3">
        <span className="h-7 w-7 rounded-lg flex items-center justify-center" style={{ background: `${accent}22`, color: accent }}>{icon}</span>

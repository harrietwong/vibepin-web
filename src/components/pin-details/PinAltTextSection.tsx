...
export type PinAltTextSectionProps = {
  /** Current alt text value. Parent owns this state. */
  value: string;
...
/**
 * Controlled alt text field for PinDetailsModal.
 * Presentational only — no API calls, no store writes.
 */
export function PinAltTextSection({ value, onChange }: PinAltTextSectionProps) {
  const { t } = useLocale();
  return (
    <>
      <div style={{ marginBottom: 7 }}>
        <span style={lbl}><span style={num}>7.</span>{t("pinDetails.altText.label")}</span>
      </div>
      <textarea
        data-testid="draft-edit-alt-text"
        value={value}
        maxLength={500}
        rows={2}
        onChange={e => onChange(e.target.value)}
        style={{ ...field, resize: "vertical" }}
        placeholder={t("pinDetails.altText.placeholder")}
      />
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
        <span style={{ fontSize: 11, color: UI.textMuted }}>{t("pinDetails.altText.helper")}</span>
        <span style={{ fontSize: 10, color: UI.textMuted }}>{value.length}/500</span>
      </div>
    </>
  );
}

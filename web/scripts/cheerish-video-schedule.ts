...
const canaryMappingId = command === "canary" || command === "stage-all"
  ? readRequiredFlag(process.argv, "--canary-mapping-id")
  : "";
...
function uploadSource(row: CheerishScheduleRow): string {
  ...
  execFileSync("ffmpeg", buildCompatibilityNormalizeCommand(sourcePath, out, sourceFacts.durationMs), { stdio: "inherit" });
...
async function scheduleRow(db: SupabaseClient, ctx:{uid:string;connectionId:string;label:string}, boards:Record<string,string>, row:CheerishScheduleRow, upload:{proxyUrl:string;width:number;height:number;durationMs:number}, override?:string) {
  ...
  if (old && !canReplaceManifestMedia({
    ...
    archivedAt: old.payload?.archivedAt, deletedAt: old.payload?.deletedAt,
    remotePinId: old.payload?.remotePinId, postedAt: old.payload?.postedAt,
...
async function main() {
  ...
  const selected = selectRowsForCommand(checked.rows, command, canaryMappingId);

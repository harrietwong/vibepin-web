/**
 * Canonical PlanEntitlements tests (task 1C-a) + planEntitlements consistency
 * tests. Two suites in one runner, over the two live modules:
 *   - src/lib/server/planEntitlements.ts — the pricing-display source of truth
 *     (pricingPlans.ts + server/entitlements.ts read it)
 *   - src/lib/planEntitlements.ts        — the metered-quota source of truth
 *     (api/billing/usage + server/usage.ts read it)
 * Both are asserted against the SAME published pricingPlans display values, so
 * enforcement and marketing can never drift on either path.
 *
...
async function main() {
  ...
  // ── Metered-quota module (src/lib/planEntitlements.ts) vs the same display ──
  // This is the module the usage metering / billing usage route reads. It must
  // agree with the very same published COMPARISON_SECTIONS cells.

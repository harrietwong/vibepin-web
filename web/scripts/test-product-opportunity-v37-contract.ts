...
for (const key of [
  "upgradeTitle", "highDemandBasis", "current7d", "previous7d", "lastUpdated",
  "productPinOnPinterest", "sourcePinOnPinterest", "moreReferences", "productLinks",
] as const) {
  assert.match(component, new RegExp(`products\\.opportunities\\.${key}`), `Product UI must expose localized ${key}`);
}
assert.match(component, /sourceTrail/, "Cards must keep a distinct evidence/source link trail");
...
const modalProductSource = component.indexOf("products.opportunities.viewProductPage");
assert.ok(
...
assert.match(component, /products\.opportunities\.additionalEvidenceNote/, "Additional references must not imply metric aggregation");
...
assert.match(component, /products\.opportunities\.savedToast/, "Save and Create Pin need distinct feedback");
...
assert.match(component, /products\.opportunities\.previousProductPage/, "Historical saved records must keep the product reference");
assert.match(component, /products\.opportunities\.pinterestReference/, "Historical saved records must keep Pinterest evidence");
assert.match(component, /products\.opportunities\.remove/, "Historical saved records must remain removable");
assert.doesNotMatch(
  server,
...
assert.match(component, /products\.opportunities\.addedToCreatePins/, "Create Pin needs distinct feedback");
const createPinHandoff = component.indexOf("await openCreatePinsWithDraft");
const createPinSuccess = component.indexOf("products.opportunities.addedToCreatePins");
assert.ok(
...
assert.match(component, /products\.opportunities\.createPinError/, "Create Pin handoff failure must be visible");
...
for (const key of ["categoryWedding", "categoryGifts", "categoryJewelry", "categoryDigital"] as const) {
  assert.match(component, new RegExp(`products\\.opportunities\\.${key}`), `${key} needs a user-facing localized label`);
}
assert.match(component, /Object\.entries\(CATEGORY_LABEL_KEYS\)/, "Category filtering must render labels while submitting stable slugs");
...
assert.match(component, /products\.opportunities\.productDetailsFrom/, "Missing-title accessible labels must remain descriptive and truthful");
...
assert.match(component, /products\.opportunities\.savedFilteredEmptyTitle/, "A saved-family mismatch must not claim that the user has no saved products");

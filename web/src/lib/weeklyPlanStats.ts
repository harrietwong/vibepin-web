...
export type WeeklyPlanStats = {
  ...
  posted:               number;
  /** Publish failures whose failed schedule belongs to the displayed week. */
  failed:               number;
};
...
export function computeWeeklyPlanStatsFromDrafts(drafts: PinDraft[], weekStart: string): WeeklyPlanStats {
  ...
  return {
    ...
    posted:               published,
    // Scope only by week, not category: Plan is one workspace publishing calendar.
    failed:               listActionablePublishFailuresInWeek(pinDraftStore.getAllDrafts(), weekStart).length,
  };
}

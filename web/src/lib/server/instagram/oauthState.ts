/**
 * Short-lived, single-use Instagram OAuth state, sealed into an HttpOnly cookie.
 *
 * Mirrors the Facebook oauthState module exactly, with two differences:
 *   - Cookie names are Instagram-specific (instagram_oauth_state /
 *     instagram_oauth_return).
 *   - The seal/unseal cipher is bound to INSTAGRAM_TOKEN_ENC_KEY via
 *     createTokenCipher(), instead of Facebook's FACEBOOK_TOKEN_ENC_KEY.
 *
 * The opaque `state` token sent to Instagram is random and contains no user data.
...
// Cipher bound to INSTAGRAM_TOKEN_ENC_KEY. Resolved lazily on first use (never at
...
export const OAUTH_RETURN_COOKIE = "instagram_oauth_return";
export const STATE_TTL_MS = 10 * 60 * 1000; // ~10 minutes

/** True when a usable INSTAGRAM_TOKEN_ENC_KEY is configured (for safe status checks). */
export function isInstagramEncryptionConfigured(): boolean {
  try {
...
/**
 * Verify the state param returned by Instagram against the sealed cookie and the

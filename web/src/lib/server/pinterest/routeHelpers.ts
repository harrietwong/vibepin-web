...
export function pinterestErrorResponse(err: unknown, extra?: Record<string, unknown>): Response {
  if (err instanceof NotConnectedError) {
    return Response.json(
      { ...extra, error: err.message, code: err.code } satisfies ErrorBody,
      { status: 409 },
    );
  }
  if (err instanceof NeedsReconnectError) {
    return Response.json(
      { ...extra, error: err.message, code: err.code, needsReconnect: true } satisfies ErrorBody,
      { status: 401 },
    );
  }
  if (err instanceof DatabaseError) {
    ...
    return Response.json(
      { ...extra, error: SAFE_MESSAGES.database_error, code: err.code } satisfies ErrorBody,
      { status: 503 },
    );
  }
  if (err instanceof ConfigurationError) {
    ...
    return Response.json(
      { ...extra, error: SAFE_MESSAGES.configuration_error, code: err.code } satisfies ErrorBody,
      { status: 500 },
    );
  }
  if (err instanceof PinterestApiError) {
    const status = err.status >= 400 && err.status < 600 ? err.status : 502;
    const body: ErrorBody = { ...extra, error: err.message, code: err.code || "pinterest_api_error" };
  ...
  return Response.json(
    { ...extra, error: SAFE_MESSAGES.internal_error, code: "internal_error" } satisfies ErrorBody,
    { status: 500 },
  );
}

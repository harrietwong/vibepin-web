...
const en = {
  ...
  "igdm.scopes.help": "After approving, you will land on Settings → Social; come back to this page. In the Instagram app also turn on Settings → Messages and story replies → Message controls → Connected tools → \"Allow access to messages\". Please use the button on this page to reconnect; reconnecting elsewhere may not request the comment/message permissions.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  "igdm.scopes.help": "授权完成后会跳到 设置 → 社交账号，请再回到本页。另外须在 Instagram App 中打开：设置 → 消息和快拍回复 → 消息控制 → 已连接的工具 →「允许访问消息」。请使用本页上的按钮重新连接；在别处重新连接可能不会请求评论/私信权限。",

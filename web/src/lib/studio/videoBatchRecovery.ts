...
function validRecord(value: unknown, scope: VideoRecoveryScope): value is VideoRecoveryRecord {
  ...
  if (r.finalized) {
    if (typeof r.finalized.proxyUrl !== "string" || typeof r.finalized.requestId !== "string" || !r.finalized.proxyUrl.startsWith("/api/storage-media?")) return false;
    try {
      const url = new URL(r.finalized.proxyUrl, "https://app.invalid");
      const path = url.searchParams.get("path");
      if (url.origin !== "https://app.invalid" || url.pathname !== "/api/storage-media" || url.searchParams.size !== 1 || !path
        || !path.startsWith(`${scope.ownerUserId}/`) || /[\\\0]|\.\.|\/\//.test(path)) return false;
    } catch { return false; }
  }
  if (r.inspection.posterUrl) {
    if (typeof r.inspection.posterUrl !== "string" || !r.inspection.posterUrl.startsWith("/api/storage-image?")) return false;
    try {
  ...
  if (r.finalized && r.posterPath && !r.attempt) return false;

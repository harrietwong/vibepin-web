 ...
 *
 * Generate ONE reference group: a batch of N references is N sequential calls to
 * this function, each requesting `setup.count` images with a single
 * `styleReference` as its style_ref. /api/generate holds a per-user
 * "active-generation" lock and returns 429 (user_generation_limit) for a
 * concurrent second call, so the groups must be serial rather than parallel.

...
/**
 * Resolve the signed-in VibePin user id (or null for anonymous browser sessions).
 * Used for usage metering — the fallback session/anon identity used by the
 * generation lock is NOT a user, so it is never metered.
 */
async function resolveGenerationUserId(req: NextRequest): Promise<string | null> {
...
function runGenerator(
  ...
  onComplete?: (result: { ok: boolean; successfulImageCount: number }) => void | Promise<void>,
...
export async function POST(req: NextRequest) {
  ...
  const moderationCostUserId = await resolveGenerationUserId(req);
  ...
  const moderation = await moderatePrompt({
    prompt: moderatedText,
    externalId: generationRequestId,
    costContext: { userId: moderationCostUserId },
  });
  ...
  const meteringUserId = await resolveGenerationUserId(req);
  ...
  try {
    return await runGenerator({
      ...
      void recordAiCost({
        ...
        provider: "linapi",
        model: process.env.LINAPI_IMAGE_MODEL || "gemini-3.1-flash-image-preview",
        operationType: "image_generation",
        requestedImageCount: imageCountClamp.requested,
        ...
        // No pixel width/height is known at this route (the provider returns
        // only image URLs) — record the requested aspect-ratio/format string as
        // the closest available signal rather than fabricating a resolution.

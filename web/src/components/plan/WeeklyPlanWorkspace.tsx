...
export function WeeklyPlanWorkspace() {
  ...
  const { t: trBase, preferences } = useLocale();
  const tr = (key: MessageKey) => trBase(key);
  const { counts: workspaceCounts } = usePinBoardDrafts("all");
  ...
  const selectViewMode = useCallback((mode: ViewMode) => {
    ...
    try {
      ...
      url.searchParams.set("planView", mode);
  ...
  useEffect(() => {
    ...
    const returnView = params.get("planView");
  ...
  const [publishFailureIdentity, setPublishFailureIdentity] = useState("");
  useEffect(() => {
    function refreshFailureCount() {
      const failures = listActionablePublishFailuresInWeek(pinDraftStore.getAllDrafts(), displayWeekStart);
      setPublishFailureCount(failures.length);
      setPublishFailureIdentity(publishFailureSetIdentity(failures));
    }
    refreshFailureCount();
  ...
  const { visibleCount: failureBannerCount, dismiss: dismissFailureBanner } = useFailureBannerDismiss(publishFailureCount, publishFailureIdentity, `plan:${displayWeekStart}`);
  ...
  return (
    ...
    <div data-testid="weekly-plan-page" style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--app-bg)" }}>
      ...
      <div style={{ padding: "12px 20px 0", background: "var(--app-surface)", flexShrink: 0 }}>
        <StudioBoardFilters value="all" counts={workspaceCounts} planActive />
      </div>
...
export default function WeeklyPlanWorkspacePage() {
  return (

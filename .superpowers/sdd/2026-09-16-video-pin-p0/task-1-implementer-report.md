...
- Implementation commits: `873a98db` (`feat(video): add v77 media upload contracts`), `644e4af1` (`fix(video): harden v77 contracts`), and `b66746b5` (`fix(video): close v77 review gaps`).
...
1. `npm run test:v77-video-media` — exit 0; `{ "verdict": "pass", "assertions": 39, "failures": [] }`.
2. `npx tsx scripts/test-content-media-model.ts` — exit 0; `22 passed, 0 failed`.
...
- v77 preflight rejects drifted provenance types/defaults, nullability, same-name altered constraints/indexes, and marked/unmarked RPC body/security/overload collisions. Rollback performs the same exact v76 validation before restoring the original image-only definitions.

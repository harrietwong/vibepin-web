...
- Implementation commits: `873a98db` (`feat(video): add v77 media upload contracts`) and `644e4af1` (`fix(video): harden v77 contracts`).
...
1. `npm run test:v77-video-media` — exit 0; `{ "verdict": "pass", "assertions": 34, "failures": [] }`.
2. `npx tsx scripts/test-content-media-model.ts` — exit 0; `21 passed, 0 failed`.
...
- Store create/load/sync-normalization and media mutation write paths keep `PinDraft.imageUrl` on a video poster and never copy a video binary URL into it.
...
- v77 preflight rejects drifted provenance types, missing owned constraints/indexes, unmarked RPC collisions, and marker/body/security/overload drift in the two v76 settlement functions. Rollback performs the same exact v76 validation before restoring the original image-only definitions.
...
- Rollback is non-destructive: it retains complete rows/schema facts, revokes write access, restores the exact original v76 image-only definitions, and permits a safe v77 reapply. Expiry errors no longer claim a state mutation that the surrounding exception would roll back.

...
- Task 3: not started.
- Task 4: complete on `codex/video-pin-p0-task4` after review repair. Shared media rendering now preserves video discriminants through Studio/Plan/Batch/Draft Details readiness and row projections; Batch previews/labels and no-poster fallback support video; Plan image thumbnail/hover behavior is retained; video controls are isolated from card/hover/lightbox activation; error fallback is polite and at least 12px. Evidence: focused Task 4 scripts, legacy image/Plan/Batch/readiness/model scripts, test registry, TypeScript, scoped ESLint, and diff check. No external mutation.
- Task 5: not started.
- Task 6: not started.
- Task 7: not started.
- Task 8: not started.
